...
then
echo "\nINSTRUCTIONS *******************************************************************************"
echo "* This is a demo of the FPGA running the bitcoin mining Algorithm.  The FPGA iteratively cycle through the algorithm looking for the solution (nonce).  Once the solution is found the FPGA will report the solution back to the Pi/"
...
echo "INFO: the hashrate (~1.8MHash/s) is intentionnaly limited to prevent the FPGA from overheating." echo "Because of the low hashrate (compared to bigger FPGA or ASIC),"
$PYTHON logibone.py
exit
fi
...
#NO LONGER SUPPORTING FULL MINING.  HARD TO KEEP UP WITH API
